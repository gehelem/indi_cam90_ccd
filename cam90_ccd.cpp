...
/** \file simpleccd.cpp
    ...
    A Cam90 device that can capture images and control temperature. It returns a FITS image to the client. To build drivers for complex CCDs, please
...
std::unique_ptr<Cam90CCD> simpleCCD ( new Cam90CCD() );
...
*******************************************************************************/
bool Cam90CCD::ISNewNumber ( const char *dev, const char *name,
                             double values[], char *names[], int n )
{
    if ( !strcmp ( dev, getDeviceName() ) )
    {
        if ( !strcmp ( name, GainNP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set gain = %d", ( int ) GainN[0].value );
            return true;
        }
        ...
        if ( !strcmp ( name, OffsetNP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set offset = %d", ( int ) OffsetN[0].value );
            return true;
        }
        ...
        /*if ( !strcmp ( name, BaudrateNP.name ) )
          {
            ...
            //IDMessage ( getDeviceName(), "Cam90 set baudrate = %d", ( int ) BaudrateN[0].value );
            return true;
          }*/
        ...
        if ( !strcmp ( name, BaudrateANP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set baudrate A = %d", ( int ) BaudrateAN[0].value );
            return true;
        }
        ...
        if ( !strcmp ( name, BaudrateBNP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set baudrate B = %d", ( int ) BaudrateBN[0].value );
            return true;
        }
        if ( !strcmp ( name, LibftditimerANP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set libftdi timerA = %d", ( int ) LibftditimerAN[0].value );
            return true;
        }
        ...
        if ( !strcmp ( name, LibftdilatencyANP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set libftdi latencyA = %d", ( int ) LibftdilatencyAN[0].value );
            return true;
        }
        if ( !strcmp ( name, LibftditimerBNP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set libftdi timerB = %d", ( int ) LibftditimerBN[0].value );
            return true;
        }
        ...
        if ( !strcmp ( name, LibftdilatencyBNP.name ) )
        {
            ...
            IDMessage ( getDeviceName(), "Cam90 set libftdi latencyA = %d", ( int ) LibftdilatencyBN[0].value );
            return true;
        }
...
bool Cam90CCD::ISNewSwitch ( const char *dev, const char *name,
...
bool Cam90CCD::Connect()
{
    ...
    if ( cameraConnect() )
    {
        ...
        cameraSetOffset ( -20 );
        cameraSetGain ( 0 );
        IDMessage ( getDeviceName(), "Cam90 connected successfully!\n" );
        ;
    ...
    {
        IDMessage ( getDeviceName(), "Cam90 connection error\n" );
        return false;
...
***************************************************************************************/
bool Cam90CCD::Disconnect()
{

    cameraDisconnect();
    return true;
    IDMessage ( getDeviceName(), "Cam90 disconnected successfully!\n" );
}
...
***************************************************************************************/
const char * Cam90CCD::getDefaultName()
{
    return "Cam90";
}
...
//                       "Baudrate", LIBFTDI_TAB, IP_RW, 0, IPS_IDLE );
    ...
    IUFillNumber ( LibftdilatencyAN, "LATENCYA", "LatencyA", "%g", 1, 50, 1, CAM90_LATENCYA );
...
*********************************************************************************************/
bool Cam90CCD::updateProperties()
{
    //cameraSetBaudrate(80);
...
void Cam90CCD::setupParams()
{
    ...
    SetCCDParams ( 3900, 2610, 16, 6.05, 6.05 );
...
***************************************************************************************/
bool Cam90CCD::StartExposure ( float duration )
{
...
***************************************************************************************/
int Cam90CCD::SetTemperature ( double temperature )
{
...
void Cam90CCD::activateCooler(bool enable)
{
